  ...
  if (ref($tree) eq 'HASH') {
    ...
    foreach my $Key ( @all_keys ) {
      ...
      if ( $Key eq '!--' && (!ref($$tree{$Key}) || ( ref($$tree{$Key}) eq 'HASH' && (keys %{$$tree{$Key}}) == 1 && (defined $$tree{$Key}{CONTENT} || defined $$tree{$Key}{content}) ) ) ) {
        ...
        if    ($tp == 1 || $tp == 4) { # added second part of condition here, Evelyn Richter, Aug 2009
		  ...
		  # commented out to avoid base64 coding of utf-8 attribute values, Evelyn Richter, Aug 2009
          #elsif ($tp == 4) {
